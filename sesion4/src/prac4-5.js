...
const videoGeometry = new THREE.PlaneGeometry(160, 90);
const videoMesh = new THREE.Mesh(videoGeometry, videoMaterial);
videoMesh.position.set(0, 45, -50);
scene.add(videoMesh);
...
document.body.addEventListener('keydown', (event) => {
    ...
    if (event.code == spaceKeyCode && intersectedObject) {
        ...
        const isRight = intersectedObject.name === 'Caja Derecha';
        
        if (isLeft) {
            video.play();
            intersectedObject.material[0].map = textura_on;
            intersectedObject.material[0].needsUpdate = true;
            rightCube.material[1].map = textura_off;
            rightCube.material[1].needsUpdate = true;
        } else if (isRight) {
            video.pause();
            intersectedObject.material[1].map = textura_on;
            intersectedObject.material[1].needsUpdate = true;
            leftCube.material[0].map = textura_off;
            leftCube.material[0].needsUpdate = true;
        }
    }
}, false);
